...
public class CitaService {
    ...
    public String Reservar (Usuario usuario, Long medicoId, LocalDate fecha, LocalTime hora) {
        ...
        if (usuario == null || medicoId == null || fecha == null || hora == null) {
        ...
        if (medicoExistente.getDisponibilidadHoraria() == null || medicoExistente.getDisponibilidadHoraria().isEmpty()) {
            return "Error: el médico no tiene horarios disponibles definidos";
        }
        ...
        //Verificar espacio no ocupado (RN1 + RN2)
        Optional<Cita> citaExistente = citaRepository.findByMedicoIdAndFechaAndHoraAndEstadoNot(medicoId, fecha, hora, "CANCELADA");
        if (citaExistente.isPresent()) {
            return "Error: el espacio ya está ocupado";
        }

        //Verificar que los usuarios no tengan citas a la misma hora (RN6)
        ...
        // Crear cita en estado PENDIENTE (RN3)
        Cita cita = new Cita();
